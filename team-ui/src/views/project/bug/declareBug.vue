...
function getDeclareList() {
  getBugDeclare(queryParams.value).then(res => {
...
function showNotification() {
  ElNotification({
    type: 'warning',
    title: '提示',
    message: h('i', {style: 'color: teal'}, '该BUG已被指定，无法修改。'),
  })
}
...
function handleDelete(row) {
  if (row.assignTo !== null) {
    ...
    proxy.$confirm('此操作删除该BUG, 是否继续?', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(() => {
      removeBug(row.id).then(res => {
        if (res.code === 200) {
...
<template>
  <div class="app-container">
    ...
    <el-table :data="declareList">
      ...
      <el-table-column label="指定人" prop="assignTo" width="100">
        <template v-slot="{row}">
          <span v-if="row.assignTo !== null">{{ row.assignToName }}</span>
      ...
      <el-table-column label="操作" align="center">
        <template v-slot="{row}">
          ...
          <el-button v-if="row.status === 0 && row.assignTo === null" type="danger" @click="handleDelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <pagination
